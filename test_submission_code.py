...
class NatureCNN(nn.Module):
    ...
    def __init__(self, input_shape, output_dim):
        ...
        self.linear = nn.Sequential(
            nn.Linear(n_flatten+obf_shape, 2048),
            nn.ReLU(),
            nn.Linear(2048, 1024),
            nn.ReLU(),
    ...
    def forward(self, observations: th.Tensor, obf_obs:th.Tensor) -> th.Tensor:
        return self.linear(th.cat((self.cnn(observations), obf_obs), 1))
...
class MineRLAgent():
    ...
    def load_agent(self):
        ...
        self.action_centroids = np.load('train/centroids_final_actions_2envs.npy')
        self.num_actions = self.action_centroids.shape[0]

        self.network = NatureCNN_obf((3, 64, 64), 64, self.num_actions).cuda()
        self.network.load_state_dict(th.load('train/prophet_iter_115_0.pth'))
    ...
    def run_agent_on_episode(self, single_episode_env: Episode):
        ...
        while not done:
            ...
            #   - Normalize image
            state_pov = th.from_numpy(state['pov'].transpose(2, 0, 1)[None].astype(np.float32) / 255).cuda()
            state_obf = th.from_numpy(state['vector'].reshape(1, -1).astype(np.float32)).cuda()
            # Turn logits into probabilities
            probabilities = th.softmax(self.network(state_pov, state_obf), dim=1)[0]
            # Into numpy
            ...
            state, reward, done, info = env.step(minerl_action)
            total_reward += reward
            steps += 1
